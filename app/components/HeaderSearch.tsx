...
const HeaderSearch = ({ onSearchSelect, placeholder = "Search for serums, creams...", autoFocus = false }: HeaderSearchProps) => {
        ...
        <div ref={searchRef} className="relative w-full group">
            ...
            {showResults && (
                <div className="absolute top-full left-0 right-0 mt-3 p-2 bg-white dark:bg-[#2a161d] shadow-xl rounded-2xl overflow-hidden border border-[#f4f0f2] dark:border-[#3a2228] z-50">
                    {loading ? (
                        <div className="p-4 text-center text-sm text-text-muted-light dark:text-text-muted-dark">Searching...</div>
                    ) : results.length > 0 ? (
                        <div className="flex flex-col gap-1">
                            {results.map((product) => (
                                <Link
                                    ...
                                    className="flex items-center gap-4 p-2.5 hover:bg-background-light dark:hover:bg-white/5 rounded-xl transition-all group/item"
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 mt-0.5">
                                            ...
                                            {/* <span className="text-[10px] text-[#89616f] bg-[#f4f0f2] dark:bg-white/10 px-1.5 py-0.5 rounded-md">
                                                Serum
                                            </span> */}
                                        </div>
                                    </div>
                                    <span className="material-symbols-outlined text-gray-300 group-hover/item:text-primary -translate-x-2 opacity-0 group-hover/item:translate-x-0 group-hover/item:opacity-100 transition-all text-xl">
                                        arrow_forward
                                    </span>
                                </Link>
                            ))}
                        ...
                        <div className="p-8 text-center flex flex-col items-center gap-2">
                            ...
                            <p className="text-sm font-medium text-text-muted-light dark:text-text-muted-dark">No products found</p>
                        </div>
                    )}
                </div>
            )}
